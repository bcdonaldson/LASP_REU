...
;*************************STEP 2: Calculate new prism degredation**************************
  ...
  for j=0, n_elements(data)-1 do begin
    ...
    ;this is initialized within the first for loop so that at every Sorce Day, a new calculated_prism_deg is created,
    ;otherwise it will create one one-dimmensional array of prism_deg for every Sorce Day at every wavelength
    calculated_prism_deg =[]
    
    for i=0, n_elements((*data[j]).wavelength)-1 do begin
      
      ;contains prism degredation at one SD (the current value of j) for every wavelength
      calculated_prism_deg = [calculated_prism_deg, (1d -attenuation1[i])*exp(-kappa1[i]*solar54[pos_time].solar_exp*((*data[j]).f_function[i])) +$
          attenuation1[i]*exp(-kappa1[i]*solar54[pos_time].solar_exp*((*data[j]).f_function[i])/2d)]
         
      prism_deg[j] = ptr_new({prism_deg1: calculated_prism_deg})
...
function plot_prism_deg, Inst_mode_ID
  ...
  if strlowcase(choice) eq 'wl' then begin
    ...
    for k=0, n_elements(data)-1 do begin
      ...
      prism_deg_at_chosen_wl = [prism_deg_at_chosen_wl, (*prism_deg[k]).prism_deg1[pos_wl]]
    endfor   
    ...
    for m=0, n_elements(prism_deg_at_chosen_wl)-1 do begin
      ...
      if prism_deg_at_chosen_wl[m] ge prismdeg_mean-(prismdeg_mean*0.5) and prism_deg_at_chosen_wl[m] le prismdeg_mean+(prismdeg_mean) then begin
        ;the difference permitted between each number may have to be altered
        if prism_deg_at_chosen_wl[m] ge prism_deg_at_chosen_wl[m-1]-.07 and prism_deg_at_chosen_wl[m] le prism_deg_at_chosen_wl[m-1]+.07 then begin
    ...
    lineplot, SIM_timestamp, prism_deg_at_chosen_wl, xtitle = 'Sorce Day', ytitle = 'Prism Degradation', $
      title = 'Sorce Day vs. Prism Degradation at Wavelength: '+user_wl_string + 'nm', $
      ptitle = 'Sorce Day vs. Prism Degradation at Wavelength: '+user_wl_string + 'nm for SIMA in the ' + title_name
    ...
    lineplot, SIM_timestamp_no_outliers, prismdeg_no_outliers, title = 'Sorce Day vs. Prism Degradation at Wavelength: '$
      +user_wl_string + 'nm without outliers'
...
;try to remove the data outliers
 ...
 ;the numbers below may need to be adjusted depending on the wavelength graphed
  for m=0, n_elements(f_function_at_chosen_wl)-1 do begin
   ;the distance from the mean that is permitted may need to be altered
   if f_function_at_chosen_wl[m] ge f_mean-(f_mean*25) and f_function_at_chosen_wl[m] le f_mean+(f_mean*2) then begin
    ;the difference permitted between each number may have to be altered (usualy is about 0.1)
    if f_function_at_chosen_wl[m] ge f_function_at_chosen_wl[m-1]-.7 and f_function_at_chosen_wl[m] le f_function_at_chosen_wl[m-1]+.7 then begin
 ...
 ;graph of all of the data points
  ...
  lineplot, SIM_timestamp, f_function_at_chosen_wl, xtitle = 'SIM timestamp (Sorce Days)', ytitle = 'F Function',$
    ptitle = "SIM Sorce Day vs. F' for SIMA in the " + type ;+ ' at ' + user_wl_string
     ;title = "SIM Sorce Day vs. F' for SIMA in the " + type,
     
  ;graph with the outliers removed
  device, decompose = 0
  lineplot, SIM_timestamp_no_outliers, f_function_no_outliers, title = "SIM Sorce Days vs. F' for SIMA in the" +$
     type + " at " + user_wl_string + ' without outliers'
